...
def run_vegeta(method, target, rate, duration, output_file):
    ...
    attack_command = f"echo '{method} {target}' | vegeta attack -rate={rate} -duration={duration}"
    ...
    try:
        ...
        markdown_report = convert_json_to_markdown(report_json, method, target, rate, duration)
...
def convert_json_to_markdown(report_json, method, target, rate, duration):
    ...
    markdown.append(f"| **Success Rate**        | {report_json.get('success', 0) * 100:.2f}% |")
    markdown.append(f"| **Latency (mean)**      | {report_json.get('latencies', {}).get('mean', 0) / 1e6:.2f} ms |")
    markdown.append(f"| **Latency (50th)**      | {report_json.get('latencies', {}).get('50th', 0) / 1e6:.2f} ms |")
    markdown.append(f"| **Latency (95th)**      | {report_json.get('latencies', {}).get('95th', 0) / 1e6:.2f} ms |")
    markdown.append(f"| **Latency (max)**       | {report_json.get('latencies', {}).get('max', 0) / 1e6:.2f} ms |")
    markdown.append(f"| **Bytes In (total)**    | {report_json.get('bytes_in', {}).get('total', 0)} bytes |")
    markdown.append(f"| **Bytes Out (total)**   | {report_json.get('bytes_out', {}).get('total', 0)} bytes |")
    markdown.append("")
    return "\n".join(markdown)
...
def plot_usage_from_file(file_path, output_file):
    ...
    with open(file_path, "r") as file:
        ...
        for line in file:
            ...
            for pod, usage in entry["usage"].items():
                print(entry["timestamp"], pod, usage)
                if pod not in cpu_data:
                    cpu_data[pod] = []
                    memory_data[pod] = []
                cpu_data[pod].append(usage["cpu"])
                memory_data[pod].append(usage["memory"])
    ...
    x_label = f"Time (seconds since monitoring start)\nStart: {start_time}, End: {end_time}"
    ...
    for pod, cpu_values in cpu_data.items():
      plt.plot(timestamps, cpu_values, label=f"{pod} CPU (mC)")
    ...
    plt.figure(figsize=(12, 6))
    for pod, memory_values in memory_data.items():
      plt.plot(timestamps, memory_values, label=f"{pod} Memory (MiB)")
...
def main():
    parser = argparse.ArgumentParser(description="Run Vegeta load tests and optionally plot resource usage.")
    ...
    parser.add_argument("-m", "--monitor", type=str, help="Path to the resource usage JSON file from monitor script")
