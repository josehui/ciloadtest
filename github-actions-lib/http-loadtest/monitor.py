...
def generate_markdown_report(data, output_file):
    ...
    for pod, usage in data.items():
        ...
        markdown.append(
            f"| {pod} | {min_cpu} | {max_cpu} | {min_memory} | {max_memory} |"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_file", type=str, help="File to save usage data and report"
    )
    parser.add_argument(
        "--pods",
        nargs="*",
        help="Specific pod names to monitor (default: all pods in namespace)",
    )
